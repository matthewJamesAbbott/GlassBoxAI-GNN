...
#
# Matthew Abbott 2025
# GNN Tests - Testing rust_cuda and rust_cuda_facade (Rust CUDA implementations)
#
...
FAIL=0
TOTAL=0
TEMP_DIR="./test_output_gnn_rust_cuda"
GNN_BIN="./rust_cuda/target/release/gnn_cuda"
FACADE_BIN="./rust_cuda_facade/target/release/gnn_facade_cuda"
...
# Compile both Rust CUDA GNN implementations
echo -e "${BLUE}Compiling Rust CUDA GNN implementations...${NC}"
echo ""

echo "Compiling rust_cuda (gnn_cuda)..."
(cd rust_cuda && cargo build --release 2>&1) || true

echo "Compiling rust_cuda_facade (gnn_facade_cuda)..."
(cd rust_cuda_facade && cargo build --release 2>&1) || true
...
echo ""
echo "========================================="
echo "GNN Rust CUDA Comprehensive Test Suite"
echo "========================================="
echo ""
...
echo -e "${BLUE}=== GNN Rust CUDA Binary Tests ===${NC}"
echo ""
...
run_test \
    "Create with --save flag (alternate)" \
    "$GNN_BIN create --feature=3 --hidden=16 --output=2 --mp-layers=2 --save=$TEMP_DIR/basic_gnn2.bin" \
    "Created GNN model"

run_test \
    "Create shows feature size" \
    "$GNN_BIN create --feature=5 --hidden=16 --output=2 --mp-layers=2 --save=$TEMP_DIR/feat5.bin" \
    "Feature size: 5"

run_test \
    "Create shows hidden size" \
    "$GNN_BIN create --feature=3 --hidden=32 --output=2 --mp-layers=2 --save=$TEMP_DIR/hid32.bin" \
    "Hidden size: 32"

run_test \
    "Create shows output size" \
    "$GNN_BIN create --feature=3 --hidden=16 --output=4 --mp-layers=2 --save=$TEMP_DIR/out4.bin" \
    "Output size: 4"

run_test \
    "Create shows MP layers" \
    "$GNN_BIN create --feature=3 --hidden=16 --output=2 --mp-layers=4 --save=$TEMP_DIR/mp4.bin" \
...
run_test \
    ...
    "$GNN_BIN create --feature=3 --hidden=16 --output=2 --mp-layers=2 --lr=0.01 --save=$TEMP_DIR/lr001.bin" \
    "Created GNN model"
...
run_test \
    ...
    "$GNN_BIN create --feature=3 --hidden=16 --output=2 --mp-layers=2 --activation=leaky-relu --save=$TEMP_DIR/act_lrelu.bin" \
    "Created GNN model"
...
run_test \
    "Create with CrossEntropy loss (not supported)" \
    "$GNN_BIN create --feature=3 --hidden=16 --output=2 --mp-layers=2 --loss=crossentropy --save=$TEMP_DIR/loss_ce.bin 2>&1 || true" \
    "error:"
...
echo -e "${BLUE}Group: Save & Load${NC}"

run_test \
    "Save command works" \
    "$GNN_BIN save --model=$TEMP_DIR/basic_gnn.bin --output=$TEMP_DIR/saved_model.bin" \
    "Model saved"

check_file_exists \
    "Saved model exists" \
    "$TEMP_DIR/saved_model.bin"

run_test \
    "Load and verify saved model" \
    "$GNN_BIN info --model=$TEMP_DIR/saved_model.bin" \
    "Model Information"
...
run_test \
    ...
    "$GNN_BIN pagerank --model=$TEMP_DIR/basic_gnn.bin --damping=0.85 --iterations=20" \
    "PageRank"

run_test \
    "PageRank with different damping" \
    "$GNN_BIN pagerank --model=$TEMP_DIR/basic_gnn.bin --damping=0.9 --iterations=10" \
    "PageRank"
...
run_test \
    ...
    "$GNN_BIN add-node --model=$TEMP_DIR/basic_gnn.bin --save=$TEMP_DIR/added_node.bin" \
    "Added node"

run_test \
    "Add-edge command runs" \
    "$GNN_BIN add-edge --model=$TEMP_DIR/basic_gnn.bin --edge=0,1 --save=$TEMP_DIR/added_edge.bin" \
    "Added edge"

run_test \
    "Remove-edge command runs" \
    "$GNN_BIN remove-edge --model=$TEMP_DIR/added_edge.bin --edge=0,1 --save=$TEMP_DIR/removed_edge.bin" \
    "Removed edge"
...
echo -e "${BLUE}Group: Gradient Flow Analysis${NC}"
...
run_test \
    "Gradient-flow shows layer info" \
    "$GNN_BIN gradient-flow --model=$TEMP_DIR/basic_gnn.bin" \
    "Layer"
...
run_test \
    ...
    "$GNN_BIN info --model=nonexistent.bin 2>&1 || true" \
    "Error"

run_test \
    "Create missing --feature" \
    "$GNN_BIN create --hidden=16 --output=2 --mp-layers=2" \
    "error:"

run_test \
    "Create missing --hidden" \
    "$GNN_BIN create --feature=3 --output=2 --mp-layers=2" \
    "error:"

run_test \
    "Create missing --output" \
    "$GNN_BIN create --feature=3 --hidden=16 --mp-layers=2" \
    "error:"

run_test \
    "Create missing --mp-layers" \
    "$GNN_BIN create --feature=3 --hidden=16 --output=2" \
    "error:"
...
run_test \
    "Add-edge missing --edge" \
    "$GNN_BIN add-edge --model=$TEMP_DIR/basic_gnn.bin" \
    "error:"
...
run_test \
    "Facade shows degree command in help" \
    "$FACADE_BIN help" \
    "degree"
...
run_test \
    ...
    "$GNN_BIN create --feature=6 --hidden=48 --output=4 --mp-layers=3 --lr=0.002 --activation=leaky-relu --loss=bce --save=$TEMP_DIR/workflow1.bin && $GNN_BIN info --model=$TEMP_DIR/workflow1.bin" \
    "Feature size: 6"
...
# ============================================
# Type Safety & Memory Safety Tests (Rust-specific)
# ============================================

echo -e "${BLUE}Group: Rust Type Safety & Memory Safety${NC}"

run_test \
    "Invalid feature size (negative)" \
    "$GNN_BIN create --feature=-1 --hidden=16 --output=2 --mp-layers=2 --save=$TEMP_DIR/invalid.bin 2>&1 || true" \
    "error"

run_test \
    "Invalid hidden size (zero)" \
    "$GNN_BIN create --feature=3 --hidden=0 --output=2 --mp-layers=2 --save=$TEMP_DIR/invalid.bin 2>&1 || true" \
    "Hidden size: 0"

run_test \
    "Large node index handled gracefully" \
    "$GNN_BIN degree --model=$TEMP_DIR/basic_gnn.bin --node=999999 2>&1 || true" \
    "degree"

run_test \
    "Invalid edge format" \
    "$GNN_BIN add-edge --model=$TEMP_DIR/basic_gnn.bin --edge=invalid --save=$TEMP_DIR/invalid.bin 2>&1 || true" \
    "Error"
...
echo "========================================="
echo "GNN Rust CUDA Implementation Summary"
echo "========================================="
echo ""
echo "Tested Implementations:"
echo "  • rust_cuda/gnn_cuda (Rust CUDA GNN)"
echo "  • rust_cuda_facade/gnn_facade_cuda (Rust Facade Pattern GNN)"
echo ""
...
echo "  ✓ Stress Tests"
echo "  ✓ Rust Type Safety & Memory Safety"
echo ""
